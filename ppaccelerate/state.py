    ...
    **Available attributes:**
    ...
    def __init__(self, cpu: bool = False, **kwargs):
        ...
        if not self.initialized:
            ...
            local_rank = int(kwargs.pop("LOCAL_RANK", -1))
            env_local_rank = int(os.environ.get("PADDLE_RANK_IN_NODE", os.environ.get("LOCAL_RANK", -1)))
            if env_local_rank != -1 and env_local_rank != local_rank and paddle.distributed.get_world_size() > 1:
                local_rank = env_local_rank
            
            if local_rank != -1 and not cpu:
    ...
    def split_between_processes(self, inputs: list | tuple | dict | paddle.Tensor, apply_padding: bool = False):
        ...
        def _split_values(inputs, start_index, end_index):
            if isinstance(inputs, (list, tuple, paddle.Tensor)):
                ...
                if apply_padding:
                    if isinstance(result, paddle.Tensor):
                        from ppaccelerate.utils import pad_across_processes, send_to_device
