...
def offload_state_dict(save_dir: Union[str, os.PathLike], state_dict: Dict[str, paddle.Tensor]):
    """
...
class OffloadedWeightsLoader(Mapping):
    ...
    Args:
        state_dict (`Dict[str, paddle.Tensor]`, *optional*):
    ...
    def __init__(
        self,
        state_dict: Dict[str, paddle.Tensor] = None,
    ...
    def __getitem__(self, key: str):
        ...
        if weight_info.get("safetensors_file") is not None:
            ...
            if "dtype" in weight_info:
                return tensor.to(getattr(paddle, weight_info["dtype"]))
            else:
                return tensor
...
def extract_submodules_state_dict(state_dict: Dict[str, paddle.Tensor], submodule_names: List[str]):
    """
    Extract the sub state-dict corresponding to a list of given submodules.

    Args:
        state_dict (`Dict[str, paddle.Tensor]`): The state dict to extract from.
