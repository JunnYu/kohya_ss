...
def recursively_apply(func, data, *args, test_type=is_torch_tensor, error_on_other_type=False, **kwargs):
    ...
    Args:
        ...
        *args:
        ...
        main_type (`type`, *optional*, defaults to `paddle.Tensor`):
...
def send_to_device(tensor, device, non_blocking=False, skip_keys=None):
    ...
    if isinstance(tensor, (tuple, list)):
        ...
        try:
            return tensor._to(device, non_blocking=non_blocking)
        except TypeError:  # .to() doesn't accept non_blocking as kwarg
            return tensor._to(device)
    else:
        return tensor
...
def get_data_structure(data):
    ...
    Args:
        data (nested list/tuple/dictionary of `paddle.Tensor`):
...
def initialize_tensors(data_structure):
    ...
    def _initialize_tensor(tensor_info):
        return paddle.ones(tensor_info.shape, dtype=tensor_info.dtype)
...
def find_batch_size(data):
    ...
    Args:
        data (nested list/tuple/dictionary of `paddle.Tensor`): The data from which to find the batch size.
    ...
    elif not isinstance(data, paddle.Tensor):
...
def listify(data):
    ...
    Args:
        data (nested list/tuple/dictionary of `paddle.Tensor`): The data from which to convert to regular numbers.

    Returns:
        The same data structure as `data` with lists of numbers instead of `paddle.Tensor`.
    """
    ...
    def _convert_to_list(tensor):
        ...
        if tensor.dtype == paddle.bfloat16:
            ...
            tensor = tensor.to(paddle.float32)
...
def _gpu_gather(tensor):
    def _gpu_gather_one(tensor):
        ...
        output_tensors = [paddle.empty_like(tensor) for _ in range(paddle.distributed.get_world_size())]
...
def _gpu_gather_object(object: Any):
    ...
    # paddle.distributed.all_gather_object(output_objects, object)
...
def concatenate(data, dim=0, axis=None):
    """
    Recursively concatenate the tensors in a nested list/tuple/dictionary of lists of tensors with the same shape.

    Args:
        data (nested list/tuple/dictionary of lists of tensors `paddle.Tensor`):
    ...
    elif not isinstance(data[0], paddle.Tensor):
        raise TypeError(f"Can only concatenate tensors but got {type(data[0])}")
    return paddle.concat(data, axis=dim)
...
def reduce(tensor, reduction="mean"):
    ...
    def _reduce_across_processes(tensor, reduction="mean"):
        ...
        if state.distributed_type == DistributedType.NO:
            ...
            paddle.distributed.all_reduce(cloned_tensor, ReduceOp.SUM)
        if reduction == "mean":
...
def convert_to_fp32(tensor):
    ...
    def _convert_to_fp32(tensor):
        return tensor._to(dtype="float32")

    def _is_fp16_bf16_tensor(tensor):
        return hasattr(tensor, "dtype") and tensor.dtype in (paddle.float16, paddle.bfloat16)
...
def find_device(data):
    ...
    Args:
        (nested list/tuple/dictionary of `paddle.Tensor`): The data we want to know the device of.
    """
