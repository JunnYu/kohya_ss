...
class AcceleratedOptimizer(paddle.optimizer.Optimizer):
    ...
    def __init__(self, optimizer, device_placement=False, scaler=None):
    ...
    def zero_grad(self, set_to_none=None):
        if self.gradient_state.sync_gradients:
            accept_arg = "set_to_none" in inspect.signature(self.optimizer.clear_grad).parameters
            if accept_arg:
                if set_to_none is None:
                    set_to_none = False
                self.optimizer.clear_grad(set_to_none=set_to_none)
            else:
