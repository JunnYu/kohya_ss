...
def save_accelerator_state(
    ...
    states["paddle_seed"] = paddle.get_rng_state()
...
def load_accelerator_state(
    ...
    **load_model_func_kwargs,
    ...
    for i, model in enumerate(models):
        ...
        models[i].set_state_dict(paddle.load(input_model_file, return_numpy=True), **load_model_func_kwargs)
    ...
    for i, opt in enumerate(optimizers):
        ...
        optimizer_state = paddle.load(input_optimizer_file, return_numpy=True)
    ...
    for i, scheduler in enumerate(schedulers):
        ...
        scheduler.set_state_dict(paddle.load(input_scheduler_file))
    logger.info("All scheduler states loaded successfully")

    # GradScaler state
    if scaler is not None:
        input_scaler_file = os.path.join(input_dir, SCALER_NAME)
        scaler.set_state_dict(paddle.load(input_scaler_file))
        logger.info("GradScaler state loaded successfully")

    # Random states
    try:
        states = paddle.load(os.path.join(input_dir, f"{RNG_STATE_NAME}_{process_index}.pkl"))
        random.setstate(states["random_state"])
        np.random.set_state(states["numpy_random_seed"])
        paddle.set_cuda_rng_state(states["paddle_seed"])
...
def save_custom_state(obj, path, index: int = 0):
    ...
    paddle.save(obj.state_dict(), save_location)
