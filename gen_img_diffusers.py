    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.safety_checker is not None:
            safety_checker_input = self.feature_extractor(self.numpy_to_pil(image), return_tensors="pd").to(self.device)
...
class MakeCutouts(paddle.nn.Layer):
...
def get_weighted_text_embeddings(
    ...
    **kwargs,
    ...
    prompt_tokens = torch.tensor(prompt_tokens, dtype=paddle.int64, device=pipe.device)
    if uncond_prompt is not None:
        ...
        uncond_tokens = torch.tensor(uncond_tokens, dtype=paddle.int64, device=pipe.device)
