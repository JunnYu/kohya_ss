...
def interrogate(args):
  weights_dtype = paddle.float16

  # いろいろ準備する
  print(f"loading SD model: {args.sd_model}")
  args.pretrained_model_name_or_path = args.sd_model
  args.vae = None
  text_encoder, vae, unet, _ = train_util._load_target_model(args,weights_dtype)
  ...
  text_encoder.to(dtype=weights_dtype)
  text_encoder.eval()
  unet.to(dtype=weights_dtype)
  ...
  network.to( dtype=weights_dtype)
  network.eval()
  ...
  for i, (orig_emb, lora_emb) in enumerate(zip(orig_embs, tqdm(lora_embs))):
    diff = paddle.mean(paddle.abs(orig_emb - lora_emb))
